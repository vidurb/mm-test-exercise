...
app.post('/',multerUpload.single('image'), async(req, res, next) => {
  try {
    ...
    const [img100, img200] = await Promise.all([resize(req.file.buffer, 100, 100), resize(req.file.buffer, 200, 200)]);
    await Promise.all([s3upload(img100, key100), s3upload(img200, key200)]);
    const [url100, url200] = await Promise.all([getUrl(key100), getUrl(key200)]);
    res.json({
      '100px': url100,
      '200px': url200, 
    })  
...
async function getUrl(key) {
  return await getSignedUrl(s3, new GetObjectCommand({Bucket: process.env.S3_BUCKET, Key: key}), {expiresIn: 86400})
}

app.listen(port, () => {console.log(`mm-test-exercise running on port ${port} using bucket ${process.env.S3_BUCKET}`)})